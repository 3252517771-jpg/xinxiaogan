...
### 原则2：前端优先，Mock先行（v1.0 存续条款）

第一期**不给后端写真实业务逻辑**。API 层只返回 mock 数据，后端只跑通注册/登录/JWT。真实 API 替换＝只改 `BASE_URL`。

> ⚠️ v2.0 更新：ML 风险预测、行为识别、DeepSeek AI 建议、推送通知**属于后端真实业务逻辑**，不受此条款限制。涉及真实后端能力时，请直接写 service 层代码。但**所有前端展示数据仍然走 Mock 优先原则**——后端 service 先确保返回正确格式，前端不依赖真实后端运行。
